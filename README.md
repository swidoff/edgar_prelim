...
The committed sqlite database file contains the following preliminary income statement items and ratios for preliminary announcements from the largest 375 US banks through 2019Q1 (verify figure):
...
* Identifies those that are preliminary announcements 
     * 8-Ks are filed for a variety of reasons
* Extracts all tables, their titles and the table units 
     * Titles and even units can be tricky to identify
* Filters tables to those whose titles are relevant to the items
     * Items are declared through metadata with regex patterns
* Conforms the tables to a standardized format 
     * Removes irrelevant rows/columns
     * Collapses header rows
     * etc...
* Locates the item rows and the most recent fiscal period columns 
     * Items may have many synonyms and fiscal periods many formats
...
Both environments may take a little while to spin up.
